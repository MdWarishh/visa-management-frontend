    ...
    <div className="fade">
      ...
      {modal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:20 }}>
          <div style={{ background:'#fff', borderRadius:12, width:'100%', maxWidth:500, boxShadow:'0 25px 60px rgba(0,0,0,0.25)', overflow:'hidden', maxHeight:'90vh', overflowY:'auto' }}>

            {/* Header */}
            <div style={{ padding:'18px 22px', borderBottom:'1px solid #f3f4f6', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, background:'#fff', zIndex:1 }}>
              <h2 style={{ fontSize:16, fontWeight:700, color:'#0f172a' }}>
                {modal === 'create' ? 'Create New User' : `Edit: ${editUser?.name}`}
              </h2>
              <button onClick={closeModal} style={{ background:'none', border:'none', cursor:'pointer', color:'#9ca3af', fontSize:20, lineHeight:1 }}>✕</button>
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ padding:'20px 22px' }}>
              ...
              <div className="fg">
                ...
                <input className="fi" placeholder="User ka naam" required
                  value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} />
              </div>

              <div className="fg">
                <label>Email Address <span style={{ color:'#ef4444' }}>*</span></label>
                <input className="fi" type="email" placeholder="user@example.com" required
                  value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))}
                  readOnly={modal !== 'create'} style={modal !== 'create' ? { background:'#f9fafb', color:'#6b7280' } : {}} />
              </div>
              ...
              <div style={{ marginTop:6 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:8, marginBottom:8 }}>
                  ...
                  <div style={{ width:40, height:22, background:'#22c55e', borderRadius:20, position:'relative' }}>
